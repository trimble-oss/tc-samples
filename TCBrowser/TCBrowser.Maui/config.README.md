...
The `environments.json` file defines all available environments and should be placed in the root of the `TCBrowser.Maui` project. It contains a dictionary of environments, each with their own configuration.
...
- **WebViewerUri**: The web viewer URI (for opening files, todos, and views in browser)
...
- Windows: `%LOCALAPPDATA%\com.tcbrowser.maui\environments.json`
